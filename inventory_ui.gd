...
func _draw() -> void:
	...
	# Armor slots
	var armor_labels: Array[String] = ["Helmet", "Chest", "Legs", "Boots"]
	for i: int in 4:
		var r: Rect2 = _armor_rects[i]
		var hover: bool = _hover_slot == _armor_slot_index(i)
		_draw_slot(r, hover)
		draw_string(font, Vector2(r.position.x + SLOT_SIZE + 4, r.position.y + 18),
			armor_labels[i], HORIZONTAL_ALIGNMENT_LEFT, -1, PauseMenu._fs(14), LABEL_COL)
		if inv != null and inv.armor_ids[i] != 0:
			_draw_item_centered(r, inv.armor_ids[i], 1)
	...
	for i: int in _storage_rects.size():
		...
		if inv != null and inv.ids[slot_idx] != 0:
			_draw_item_centered(r, inv.ids[slot_idx], inv.counts[slot_idx])
...
func _draw_tooltip(font: Font) -> void:
	...
	if name_str.is_empty():
		return
	var tw: float = float(name_str.length()) * 7.0 + 12.0
	var mp: Vector2 = get_viewport().get_mouse_position()
	var tx: float = mp.x + 12
	var ty: float = mp.y - 28
	draw_rect(Rect2(tx - 4, ty - 16, tw + 8, 22), Color(0.1, 0.1, 0.1, 0.85))
...
func _handle_left_click(slot: int, inv: Inventory, shift: bool) -> void:
	...
	if slot >= _armor_slot_index(0) and slot < _armor_slot_index(4):
		...
		if _cursor_id == 0:
			...
			if Items.is_armor(_cursor_id) and Items.get_armor_slot(_cursor_id) == ai:
